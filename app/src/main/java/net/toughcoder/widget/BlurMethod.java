 ...
 */
public class BlurMethod {
    static {
        System.loadLibrary("effectiveandroid");
    }
    ...
    public static Bitmap BlurWithNative(Bitmap in, int radius) {
        Bitmap bm;
        if (in.isMutable()) {
            bm = in;
        } else {
            bm = in.copy(in.getConfig(), true);
        }
        nativeBlur(bm, radius);
        return bm;
    }
